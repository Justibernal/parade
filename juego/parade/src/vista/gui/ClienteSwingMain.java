...
public class ClienteSwingMain extends JFrame {
  ...
  private JMenuItem miStart, miJugar, miTop;
  private JMenuItem miEstadoJugadores; // nuevo

  // renderer compartido con escala para cartas
  private final CardCellRenderer cardRenderer = new CardCellRenderer();

  // ====== NUEVO: puntajes ======
  private final JLabel puntosLbl    = new JLabel("Puntos: 0"); // debajo de pilas (tu subtotal)
  private final JLabel miniScoreLbl = new JLabel(" ");          // arriba (tira compacta con todos)
  private EstadoJuego lastState     = null;                     // último estado para el diálogo
  ...
  public ClienteSwingMain() {
    ...
    top.add(Box.createHorizontalStrut(16)); // NUEVO: mini score
    miniScoreLbl.setForeground(new java.awt.Color(0x424242));
    miniScoreLbl.setFont(miniScoreLbl.getFont().deriveFont(12f));
    top.add(miniScoreLbl);
    root.add(top, BorderLayout.NORTH);

    // ---------- Panel central (3 filas verticales) ----------
    ...
    pPilas.setBorder(new TitledBorder("Cartas Capturadas (cantidad por color)"));
    ...
    pilasList.setPrototypeCellValue(new PilaItem(modelo.core.Color.AMARILLO, 99));
    ...
    // NUEVO: subtotal de puntos (no ocupa casi espacio)
    var southPilas = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 2));
    puntosLbl.setFont(puntosLbl.getFont().deriveFont(Font.BOLD, 12f));
    southPilas.add(puntosLbl);
    pPilas.add(southPilas, BorderLayout.SOUTH);
  ...
  private JMenuBar createMenuBar() {
    ...
    // ===== NUEVO: Estado de jugadores (popup compacto)
    ver.addSeparator();
    miEstadoJugadores = new JMenuItem("Estado de jugadores…");
    miEstadoJugadores.setEnabled(false);
    miEstadoJugadores.setAccelerator(KeyStroke.getKeyStroke(
        KeyEvent.VK_E, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
    miEstadoJugadores.addActionListener(e -> showEstadoJugadores());
    ver.add(miEstadoJugadores);
  ...
  private void conectarYUnirse() {
    try {
      ...
      if (miEstadoJugadores != null) miEstadoJugadores.setEnabled(true); // NUEVO
  ...
  private static class PilaRenderer extends JLabel implements ListCellRenderer<PilaItem> {
    private static final long serialVersionUID = 1L;
    PilaRenderer() { setOpaque(true); setHorizontalAlignment(CENTER); setVerticalAlignment(CENTER); }

    @Override public Component getListCellRendererComponent(
        JList<? extends PilaItem> list, PilaItem item, int index, boolean isSelected, boolean cellHasFocus) {
      if (item != null) {
        setText(item.color.name() + " = " + item.count);
        setToolTipText(item.color.name() +
            " — sin mayoría: suma valores; con mayoría: 1 punto por carta");
        setBackground(new java.awt.Color(0xFAFAFA));
      ...
      if (isSelected) setBorder(BorderFactory.createLineBorder(new java.awt.Color(0x0097A7), 2));
      return this;
    }
  }

  // ===== helpers =====
  private static class PilaItem {
    final modelo.core.Color color; final int count;
    PilaItem(modelo.core.Color c, int n){ this.color=c; this.count=n; }
  }

  private static java.awt.Color colorToAwt(modelo.core.Color c) {
    switch (c) {
      case AZUL:     return new java.awt.Color(0x2196F3);
      case VERDE:    return new java.awt.Color(0x43A047);
      case AMARILLO: return new java.awt.Color(0xFBC02D);
      case ROJO:     return new java.awt.Color(0xE53935);
      case NEGRO:    return new java.awt.Color(0x424242);
      case MORADO:   return new java.awt.Color(0x8E24AA);
      default:       return java.awt.Color.lightGray;
    }
  }

  // ====== Binder de estado ======
  private void updateState(EstadoJuego s) {
    if (s == null) return;
    lastState = s; // NUEVO: guardar para el diálogo

    String turno = (s.turnoDe == null) ? "-" :
        (s.jugadores.get(s.turnoDe) != null ? s.jugadores.get(s.turnoDe).nombre : "?");
    turnoLbl.setText("Turno: " + turno);
    mazoLbl.setText("Mazo: " + s.cartasMazoRestantes);

    // ¿Estoy en fase final y me toca elegir 2 cartas?
    boolean enFinal = s.ultimaRonda &&
                      s.pendientesEleccion != null &&
                      s.pendientesEleccion.contains(myId);

    myTurn = (s.turnoDe != null && s.turnoDe.equals(myId));
    // En final no se puede "Jugar"; sí seleccionar y usar el botón de elección
    playBtn.setEnabled(myTurn && !s.ultimaRonda);
    if (miJugar != null) miJugar.setEnabled(myTurn && !s.ultimaRonda);
    manoList.setEnabled(myTurn || enFinal);
    finalPickBtn.setVisible(enFinal);

    turnoLbl.setForeground(myTurn ? new java.awt.Color(0x2E7D32) : java.awt.Color.DARK_GRAY);

    desfileModel.clear();
    for (Carta c : s.desfile) desfileModel.addElement(c);

    manoModel.clear();
    EstadoJuego.Jugador yo = s.jugadores.get(myId);
    if (yo != null) for (Carta c : yo.mano) manoModel.addElement(c);

    pilasModel.clear();
    if (yo != null) {
      for (java.util.Map.Entry<modelo.core.Color, java.util.List<Carta>> e : yo.pilas.entrySet()) {
        pilasModel.addElement(new PilaItem(e.getKey(), e.getValue().size()));
      }
    }

    // ===== NUEVO: subtotal mío + mini score de todos =====
    if (yo != null) {
      int misPuntos = calcularPuntosDeJugador(myId, s);
      puntosLbl.setText("Puntos: " + misPuntos);
    }
    StringBuilder mini = new StringBuilder("Pts: ");
    var orden = new java.util.ArrayList<>(s.jugadores.keySet());
    orden.sort((a,b) -> {
      var ja = s.jugadores.get(a); var jb = s.jugadores.get(b);
      String na = (ja!=null && ja.nombre!=null)? ja.nombre : a;
      String nb = (jb!=null && jb.nombre!=null)? jb.nombre : b;
      return na.compareToIgnoreCase(nb);
    });
    boolean firstP = true;
    for (String id : orden) {
      var j = s.jugadores.get(id);
      String nombre = (j!=null && j.nombre!=null)? j.nombre : id;
      int pts = calcularPuntosDeJugador(id, s);
      if (!firstP) mini.append(" · ");
      boolean soy = id.equals(myId);
      mini.append(soy ? "vos" : nombre).append("=").append(pts);
      firstP = false;
    }
    miniScoreLbl.setText(mini.toString());

    // Jugadores: azul = vos, rojo = rivales
    StringBuilder sb = new StringBuilder("<html>Jugadores: ");
    boolean first = true;
    for (var ent : s.jugadores.entrySet()) {
      String id = ent.getKey();
      EstadoJuego.Jugador j = ent.getValue();
      if (!first) sb.append(" · ");
      String nombre = j.nombre != null ? j.nombre : id;
      boolean soyYo = id.equals(myId);
      String colorHex = soyYo ? "#1565C0" : "#C62828";
      sb.append("<b><span style='color:").append(colorHex).append("'>")
        .append(nombre).append(soyYo ? " (vos)" : "")
        .append("</span></b>");
      first = false;
    }
    sb.append("<br><span style='font-size:10px;color:#666'>Verde = turno actual · Azul = vos · Rojo = rivales</span>");
    sb.append("</html>");
    jugadoresLbl.setText(sb.toString());

    if (s.juegoTerminado && !finMostrado) {
      finMostrado = true;
      mostrarPopupFinal(s);
    }
  }

  // ======= Helpers de puntuación + diálogo =======
  private int calcularPuntosDeJugador(String jugadorId, EstadoJuego s) {
    EstadoJuego.Jugador j = s.jugadores.get(jugadorId);
    if (j == null || j.pilas == null) return 0;

    int total = 0;
    int cantJug = s.jugadores.size();

    for (var entry : j.pilas.entrySet()) {
      modelo.core.Color color = entry.getKey();
      var mias = entry.getValue();
      int misCant = (mias == null) ? 0 : mias.size();
      if (misCant == 0) continue;

      int maxOtro = 0;
      for (var ent : s.jugadores.entrySet()) {
        if (ent.getKey().equals(jugadorId)) continue;
        var pil = ent.getValue().pilas.get(color);
        int c = (pil == null) ? 0 : (pil.size());
        if (c > maxOtro) maxOtro = c;
      }

      boolean mayoria;
      if (cantJug == 2) {
        // mayoría especial 2 jugadores: >= (otro + 2)
        mayoria = misCant >= maxOtro + 2;
      } else {
        // 3+ jugadores: comparten mayoría quienes empatan el máximo
        int maxGlobal = Math.max(misCant, maxOtro);
        mayoria = (misCant == maxGlobal);
      }

      if (mayoria) {
        total += misCant;
      } else {
        for (Carta c : mias) total += c.getValor();
      }
    }
    return total;
  }

  private void showEstadoJugadores() {
    EstadoJuego s = lastState;
    if (s == null) {
      JOptionPane.showMessageDialog(this, "No hay estado disponible.", "Estado de jugadores",
          JOptionPane.INFORMATION_MESSAGE);
      return;
    }
    StringBuilder html = new StringBuilder();
    html.append("<html><body style='width:520px;font-family:sans-serif'>")
        .append("<h3>Estado de jugadores</h3>");
    ...
    for (var ent : s.jugadores.entrySet()) {
      ...
      html.append("<div style='margin:6px 0;padding:6px;border:1px solid #ddd;border-radius:6px;'>");
      html.append("<b>").append(nombre).append(id.equals(myId) ? " (vos)" : "").append("</b>")
          .append(" — Puntos: <b>").append(pts).append("</b><br>");

      if (j.pilas == null || j.pilas.isEmpty()) {
        html.append("<i>Sin cartas capturadas</i>");
      } else {
        html.append("<span style='font-size:12px;color:#333'>");
        boolean first = true;
        for (var e : j.pilas.entrySet()) {
          int cant = e.getValue().size();
          if (!first) html.append(" · ");
          html.append(e.getKey().name()).append("=").append(cant);
          first = false;
        }
        html.append("</span>");
      }
    ...
    JOptionPane.showMessageDialog(this, html.toString(),
        "Estado de jugadores", JOptionPane.INFORMATION_MESSAGE);
  }

  // "log" ahora escribe como info en el chat (gris)
  private void log(String msg) {
    chatArea.append("[info] " + msg + "\n");
    chatArea.setCaretPosition(chatArea.getDocument().getLength());
  }

  private void invokeRemote(RemoteRunnable r) {
    try { r.run(); }
    catch (Exception ex) {
      ex.printStackTrace();
      JOptionPane.showMessageDialog(this, ex.getMessage(), "Error remoto",
          JOptionPane.ERROR_MESSAGE);
    }
  }

  private void mostrarPopupFinal(EstadoJuego s) {
    String resumen = (lastResumen != null) ? lastResumen : "Partida finalizada.";
    String titulo = "Fin de juego";
    int idx = resumen.indexOf("Ganador:");
    if (idx >= 0) {
      String ganador = resumen.substring(idx + "Ganador:".length()).trim();
      titulo = (myName != null && ganador.equalsIgnoreCase(myName)) ? "¡Ganaste!" : "Perdiste";
    }
    JOptionPane.showMessageDialog(this, resumen, titulo, JOptionPane.INFORMATION_MESSAGE);
  }

  private void showReglasRapidas() {
    String html =
        "<html><body style='width:420px;font-family:sans-serif'>" +
        "<h3>Reglas rápidas – Parade</h3>" +
        "<ol>" +
        "<li><b>Jugar carta:</b> poné 1 carta de tu mano al final del <b>parade</b>.</li>" +
        "<li><b>Ignorar N cartas:</b> con N = valor de tu carta, ignorás las N cartas más cercanas.</li>" +
        "<li><b>Capturar:</b> desde la carta N+1 hacia el otro extremo, capturás toda carta " +
        "del <b>mismo color</b> o con <b>valor ≤ N</b>. Van a tus pilas por color.</li>" +
        "<li><b>Robar:</b> robá 1 carta del mazo (si queda).</li>" +
        "</ol>" +
        "<p><b>Fin:</b> sin mazo o alguien tiene todos los colores. Cada jugador mueve 2 cartas de su mano a pilas.</p>" +
        "<p><b>Puntos por color:</b> si tenés mayoría → 1 por carta; si no, suma de valores. <br>" +
        "<i>Menos puntos gana.</i></p>" +
        "</body></html>";
    JOptionPane.showMessageDialog(this, html, "Reglas rápidas", JOptionPane.INFORMATION_MESSAGE);
  }

  @FunctionalInterface private interface RemoteRunnable { void run() throws Exception; }

  // ====== main ======
  public static void main(String[] args) {
    SwingUtilities.invokeLater(() -> new ClienteSwingMain().setVisible(true));
  }
}
